  ...
  return (
    <div className={`grid w-full gap-2 border-2 border-line bg-surface p-2 sm:p-3 ${isPlaying ? 'grid-cols-[minmax(0,1fr)_auto]' : 'grid-cols-[3.25rem_minmax(0,1fr)] sm:grid-cols-[1fr_auto]'}`}>
      <div className="flex min-h-11 flex-col justify-center">
        <span className={isPlaying ? 'flex items-center gap-1.5 font-display text-lg uppercase' : 'text-xs font-bold uppercase tracking-wide'}>
          {isPlaying ? <span className={`h-3 w-3 shrink-0 border-2 border-ink ${mode === 'work' ? 'bg-mint' : 'bg-blue'}`} aria-hidden="true" /> : null}
          {label}
        </span>
        {!isPlaying ? (
      ...
      {isPlaying ? (
        ...
        <div className="grid grid-cols-[auto_auto_minmax(3.25rem,1fr)_auto_auto] items-center gap-1">
          ...
          <input
            className={`h-11 min-w-0 border-2 border-ink text-center text-sm font-bold outline-none focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-blue ${mode === 'work' ? 'bg-mint' : 'bg-blue'}`}
            type="number"
